...
session("perform_stats_tests", False)
...
if session("assays") is not None and session("normalisers") is not None:
    ...
    ctrl.setup_drop_rel(controls_right)
    ...
    if session("perform_calibration"):
        show_calibration = controls_left.checkbox("Show Calibration Line Plot", value=True, help="If any new qPCR efficiencies were computed during calibration, visualise the regression lines in a figure.")
    else:
    ...
    run_statistics = controls_left.checkbox(
        "Perform statistics",
        value=True,
        help="Perform statistical tests on the data. Note, this only affects the `Preview` figure (not the Filter figure!). When this box is checked, you can edit the figure settings and do not need to push the `Run analysis` button again, the figures will automatically update!",
    )
    session("perform_stats_tests", run_statistics)

    controls_left.markdown("---")
    run_button = controls_left.button("Run Analysis", help="Compute Delta-Delta-Ct")

    if run_statistics:
        # setup statistical tests
        ctrl.setup_statistical_tests(advanced)
    ...
    if analysis_was_run:
        ...
        core.show_anova_table(results_container)
        core.show_ttest_table(results_container)
        ...
        if session("test_results"):
            ctrl.setup_tests_download(download_buttons[-2])
