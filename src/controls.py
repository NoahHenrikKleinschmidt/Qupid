...
def upload_multiple_files(container, build=False):
    ...
    if build:
        ...
        assay_files = container.file_uploader(
            "Upload Input Assays", type=["csv", "xlsx"], accept_multiple_files=True, help="Upload csv files here for all assays that shall be treated as samples-of-interest.\nPlease, upload a separate file for each assay."
        )

        norm_files = container.file_uploader(
            "Upload Normaliser Assays", type=["csv", "xlsx"], accept_multiple_files=True, help="Upload csv files here for all assays that shall be treated as normalisers.\nPlease, upload a separate file for each assay."
        )
        # and save to session
        if assay_files != []:
...
def setup_replicates_and_names(container, allow_infer=True):
    ...
    if allow_infer:
        ...
        infer = container.checkbox("Infer replicates", help="Select if your replicates are identically labelled and you wish to use automatically inferred replicates.", value=True)
    if not infer:
        ...
        names = container.text_input("Specify group names", help="Specify the names of your groups of replicates, or leave blank to use default names (group0, group1, ...).", placeholder="untreated, ...")
...
def setup_csv_delimiter(container):
    ...
    delimiter = container.radio(
        "Select a delimiter", help="Select which delimiter your file uses. Classic csv files use a comma `,` but some use a semicolon `;`. If you're not sure about your file(s), open them in some regular text editor to check.", options=options
    )
    # save to session
...
def upload_single_file(container, build=False):
    ...
    if build:
        try:
            ...
            assay_files = container.file_uploader("Upload Input File", type=["csv", "xlsx"], accept_multiple_files=False, help="Upload one decorated `csv` or `xlsx` file here for all assays and normalisers.")
            # and save to session
            session("assay_files", assay_files)
...
def setup_multi_assay_file(container, build=False):
    ...
    if build:
        ...
        if assay_pattern == "other":
            assay_pattern = container.text_input("Specify assay pattern", placeholder="Assay: ([a-zA-Z0-9 ,\-]+)", help="Specify a valid `regex` attern with _one_ single capturing group that captures the assay name.")
        ...
        if session("assay_files") is not None:
            ...
            if reader.is_excel():
                ...
                if reader.is_multisheet():
                    ...
                    is_multisheet = container.checkbox("Read all sheets from my file", "Select this if your file contains multiple sheets from which you wish to read data")
                    # save to session
...
def setup_transpose_controls(container):
    ...
    """
    transposed = container.checkbox("Read transposed", help="Select this if your assays are not above one another but next to one another.")
...
def setup_figure_type(container):
    ...
    figure_type = container.selectbox("Select Figure Type", options=options, help="Either choose a bar plot with mean and stdev for each group or a dot plot with the individual replicate values.")
    ...
    if figure_type == "Dots":
        show_violins = container.checkbox("Show violins", value=False, help="Show kde curves of the distributions of the shown datapoints.")
        return show_violins
...
def setup_subplot_type(container):
    ...
    subplot_type = container.selectbox("Select Subplot Handles", options, help="Select wether to plot `assays` as separate subplots (with groups on the x-axis) or to plot `groups` as separate subplots (with assays on the x-axis).")
...
def setup_normaliser_mode(container):
    ...
    if normalisation_mode == "permutative":
        ...
        allow_replace = container.checkbox("Allow replacement", help="Select this if you wish to allow replacement during the permutation process.", value=False)
...
def setup_plotting_kwargs(container):
    ...
    plotting_kwargs = [i[:-1] if i.endswith(",") else i for i in plotting_kwargs.split("\n")]
...
def setup_drop_groups_selection(container):
    ...
    invert_selection = container.checkbox("Invert selection", help="Use this to invert your selection. In this case only the selected groups will be plotted.")
    if invert_selection:
...
def setup_drop_rel(container):
    ...
    for plotting
    """
    drop_rel = container.checkbox("Drop '_rel_'", help="This will drop the `_rel_{normaliser}` part of the composite `{assay}_rel_{normaliser}` ids, and thus restore the original assay id when plotting.")
...
def setup_results_downloads(container):
    ...
    container.download_button("Download Results", rep_results.to_csv(index=False), mime="text/csv", help="Download the final Delta-Delta-Ct results retaining all individual replicate values.")
...
def setup_summarised_download(container):
    ...
    container.download_button("Download Summarized Results", stats_results.to_csv(index=False), mime="text/csv", help="Download the final Delta-Delta-Ct results results summarized to mean and stdev of each replicate group.")
...
def vet_all_assays_grouped():
    ...
    if len(no_groups) > 0:
        ...
        if not same_length:
            st.error(
                "At least one of the above assays does not have the same number of replicate entries as the others! Make sure that all assays have the same length (add dummy groups if necessary). Or, manually adjust replicate identifiers to allow automatic inference of replicates."
            )
...
def setup_calibration_Settings(container):
    ...
    efficiency_reference_file = container.file_uploader(
        "Upload an efficiency reference file", help="If you have pre-computed qPCR primer efficiencies stored in a `csv` file you can upload them here. Note, this file must contain exactly two (named) columns containing assay ids and efficiencies."
    )
    session("efficiency_reference_file", efficiency_reference_file)
    remove_calibrators = container.checkbox("Remove calibrator replicates", help="Remove all calibrator replicates from the Assays after calibration.", value=True)
    session("remove_calibrators", remove_calibrators)

    ignore_uncalibratable = container.checkbox(
        "Ignore non-calibratable", help="Ignore any assays that can neither be newly calibrated nor be assigned an efficiency. If un-selected, Qupid will raise an Error if any assay cannot be calibrated.", value=True
    )
